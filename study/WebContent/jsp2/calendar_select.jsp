...
<%
	...
	// 클라이언트에서 넘어온 년, 월 받기
	String yearSel = request.getParameter("yearSel");	// 넘어오는 year값
	String monthSel = request.getParameter("monthSel"); 	// 넘어오는 month값
	...
	if(monthSel != null) {
		...
		month = Integer.parseInt(monthSel);
	}
...
<script type="text/javascript">
	function selectdate(){
		document.myForm.action = "calendar_select.jsp?year=<%=year%>&month=<%=month%>";
...
<table align="center" width="210" cellpadding="2" cellspacing="1">
	<tr>
		<td align="center">
			<select name="yearSel" onChange="selectdate()">
			<%
			...
			<select name="monthSel" onchange="selectdate()">
			<%
				for(int i=1; i<=12; i++){
					if(i==month){
						out.print("<option value='"+i+"' selected='selected'>"+i+"</option>");
